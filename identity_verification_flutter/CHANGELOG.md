...
- **Federated Plugin 迁移**：仓库重组为四包并列结构（对齐 flutter/packages url_launcher）
  - `identity_verification_flutter/`（app-facing，对外 API）
  - `identity_verification_flutter_platform_interface/`（平台接口 + MethodChannel 默认实现）
  - `identity_verification_flutter_android/`（Android 原生 + dartPluginClass）
  - `identity_verification_flutter_ios/`（iOS 原生 + dartPluginClass）
- **API 命名优化**（旧名保留 deprecated 别名）：
  - `TencentIdentityVerification` → `IdentityVerification`
  - `FaceVerifyH5Result` → `FaceVerifyResult`
  - `startH5FaceVerify` → `startFaceVerify`
  - `destroyH5FaceVerify` → `destroy`
  - `parseCallbackResult` → `FaceVerifyResult.fromUri`
